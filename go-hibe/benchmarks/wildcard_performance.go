...
func NewWildcardBenchmark() *WildcardBenchmark {
	return &WildcardBenchmark{
		patternSize:   20,
		testHierarchy: []byte("waste-management_wildcard_test"),
...
func (wb *WildcardBenchmark) benchmarkNonWildcard(ctx context.Context, uri string) (*WildcardResult, error) {
	...
	encoder := hibe.NewDefaultPatternEncoder(wb.patternSize - hibe.MaxTimeLength)
	...
	pattern := make(hibe.Pattern, wb.patternSize)
	...
	start = time.Now()
	testPattern := make(hibe.Pattern, wb.patternSize)
	for i, component := range uriComponents {
...
func (wb *WildcardBenchmark) wildcardMatches(pattern1, pattern2 hibe.Pattern) bool {
...
func (wb *WildcardBenchmark) BenchmarkComplexHierarchies(ctx context.Context) ([]WildcardResult, error) {
	complexURIs := []string{
		"facility/cardiology/bin123/ecg/2023/january",
		"clinic/neurology/bin456/mri/scan001/series002",
		"research/oncology/study789/bin101/treatment/cycle3",
		"insurance/claims/provider555/bin888/diagnosis/diabetes",
		"pharmacy/prescriptions/operator222/bin999/medication/dosage",
		"lab/bloodwork/technician111/bin333/glucose/fasting",
	}
