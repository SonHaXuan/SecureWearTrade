...
import (
	...
	"github.com/ucbrise/hibe-pairing/lang/go/wkdibe"
)
...
func NewSchemeComparison() *SchemeComparison {
	return &SchemeComparison{
		testHierarchy: []byte("waste-management_hierarchy"),
		patternSize:   20,
		testMessage:   []byte("test waste-management data for HIBE comparison"),
		testURI:       "facility/bin/record",
	}
}

// BenchmarkHIBEScheme benchmarks our enhanced HIBE implementation
func (sc *SchemeComparison) BenchmarkHIBEScheme(ctx context.Context) (*ComparisonResult, error) {
	result := &ComparisonResult{
		SchemeName:      "Enhanced HIBE (SecureWearTrade)",
	...
	encoder := hibe.NewDefaultPatternEncoder(sc.patternSize - hibe.MaxTimeLength)
	...
	start := time.Now()
	pattern := make(hibe.Pattern, sc.patternSize)
...
func (sc *SchemeComparison) BenchmarkLHABEScheme(ctx context.Context) (*ComparisonResult, error) {
	...
	// Simulate LHABE performance based on literature (typically slower than HIBE)
...
func (sc *SchemeComparison) GenerateComparisonReport(results []ComparisonResult) string {
	...
	if len(results) > 0 {
		...
		hibeResult := results[0] // Assuming HIBE is first
		report += "\n=== HIBE PERFORMANCE ADVANTAGES ===\n\n"
		
		for i := 1; i < len(results); i++ {
			other := results[i]
			keyGenImprovement := float64(other.KeyGenerationTimeMs-hibeResult.KeyGenerationTimeMs) / float64(other.KeyGenerationTimeMs) * 100
			encryptImprovement := float64(other.EncryptionTimeMs-hibeResult.EncryptionTimeMs) / float64(other.EncryptionTimeMs) * 100
			memoryImprovement := float64(other.MemoryUsageKB-hibeResult.MemoryUsageKB) / float64(other.MemoryUsageKB) * 100
...
// WasteManagementSpecificBenchmark tests performance with waste-management-specific data patterns
func (sc *SchemeComparison) WasteManagementSpecificBenchmark(ctx context.Context) (*ComparisonResult, error) {
	// Test with waste-management-specific URI patterns
	waste-managementURIs := []string{
		"facility/cardiology/bin123/ecg",
		"clinic/*/bin*/vitals",
		"research/*/dataset/anonymized",
		"insurance/claims/bin*/diagnosis",
	}
	
	var totalKeyGenTime, totalEncryptTime, totalDecryptTime int64
	var totalMemory uint64
	
	for _, uri := range waste-managementURIs {
	...
	avgResult := &ComparisonResult{
		SchemeName:          "HIBE WasteManagement-Specific",
		KeyGenerationTimeMs: totalKeyGenTime / int64(len(waste-managementURIs)),
		EncryptionTimeMs:    totalEncryptTime / int64(len(waste-managementURIs)),
		DecryptionTimeMs:    totalDecryptTime / int64(len(waste-managementURIs)),
		MemoryUsageKB:       totalMemory / uint64(len(waste-managementURIs)),
		WildcardSupport:     true,
		StorageType:         "IPFS Distributed",
		TestTimestamp:       time.Now(),
	}
