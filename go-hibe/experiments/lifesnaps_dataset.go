...
// LifeSnapsDataset represents the synthetic LifeSnaps waste-management dataset
...
// Bin represents a waste-management bin
type Bin struct {
	ID              string    `json:"id"`
	Age             int       `json:"age"`
...
// WearableDevice represents a wearable waste-management device
...
// HealthRecord represents waste records and diagnoses
...
// TestScenario represents different testing scenarios for waste-management data
...
// AccessPattern represents how data is accessed in waste-management scenarios
...
func NewLifeSnapsDataset(sizeGB int, syntheticRatio float64) *LifeSnapsDataset {
	...
	binCount := calculateBinCount(sizeGB)
	deviceCount := binCount * 2 // Average 2 devices per bin
	
	dataset.Bins = generateBins(binCount)
	dataset.Devices = generateWearableDevices(deviceCount, dataset.Bins)
	dataset.VitalSigns = generateVitalSigns(dataset.Bins, dataset.Devices, sizeGB)
	dataset.Activities = generateActivities(dataset.Bins, dataset.Devices, sizeGB)
	dataset.HealthRecords = generateHealthRecords(dataset.Bins, sizeGB)
	
	return dataset
}

// generateHierarchyStructure creates realistic waste-management hierarchy patterns
func generateHierarchyStructure() []string {
	return []string{
		"facility/*/bin/*/vitals",
		"clinic/cardiology/bin/*/ecg",
		"research/*/study/*/participant/*",
		"pharmacy/*/prescription/*/medication",
		"insurance/*/claims/*/diagnosis",
		"emergency/*/bin/*/triage",
		"lab/*/test/*/results",
...
// calculateBinCount determines number of bins based on target data size
func calculateBinCount(sizeGB int) int {
	// Estimate: ~50MB of data per bin (including all records)
	avgDataPerBinMB := 50
	return (sizeGB * 1024) / avgDataPerBinMB
}

// generateBins creates synthetic bin data
func generateBins(count int) []Bin {
	bins := make([]Bin, count)
...
func generateWearableDevices(count int, bins []Bin) []WearableDevice {
	...
	for _, bin := range bins {
		for d := 0; d < bin.DeviceCount && deviceIndex < count; d++ {
...
func generateVitalSigns(bins []Bin, devices []WearableDevice, sizeGB int) []VitalSignData {
	...
	for i := 0; i < targetRecords; i++ {
		bin := bins[mathrand.Intn(len(bins))]
		
		// Find device for this bin
		...
		// Generate realistic vital signs based on bin condition
		baseHeartRate := 70
		baseTemp := 36.5
		baseBP := 120
...
func generateActivities(bins []Bin, devices []WearableDevice, sizeGB int) []ActivityData {
	avgRecordSize := 120
	...
	for i := 0; i < targetRecords; i++ {
		...
		var device WearableDevice
		for _, d := range devices {
			if d.BinID == bin.ID && contains(d.DataTypes, "steps") {
				device = d
				break
			}
		}
...
func generateHealthRecords(bins []Bin, sizeGB int) []HealthRecord {
	avgRecordSize := 500
	...
	for i := 0; i < targetRecords; i++ {
		...
		records[i] = HealthRecord{
			...
			OperatorID:     fmt.Sprintf("operator_%04d", mathrand.Intn(100)+1),
			FacilityID:   fmt.Sprintf("facility_%03d", mathrand.Intn(20)+1),
...
// CreateTestScenarios generates waste-management-specific test scenarios
func CreateTestScenarios() []TestScenario {
	return []TestScenario{
		{
			Name:         "Small Clinic",
			Description:  "Small clinic with 100 bins and basic monitoring",
			BinCount: 100,
			DeviceCount:  150,
			...
			AccessPatterns: []AccessPattern{
				{
					...
					Hierarchy:       "clinic/general/bin/*/vitals",
					Frequency:       5,
					DataSize:        50,
					Users:           []string{"operator", "nurse"},
					PermissionLevel: "read_write",
				},
				{
					Type:            "emergency_access",
					Hierarchy:       "clinic/emergency/bin/*/all",
					Frequency:       1,
					DataSize:        500,
					Users:           []string{"emergency_operator"},
		...
		{
			Name:         "Large Facility",
			Description:  "Large facility with 10,000 bins and multiple departments",
			BinCount: 10000,
			DeviceCount:  25000,
			...
			AccessPatterns: []AccessPattern{
				{
					...
					Hierarchy:       "facility/*/bin/*/records",
					Frequency:       50,
					DataSize:        200,
				...
				},
				{
					...
					Hierarchy:       "facility/research/*/anonymized",
					Frequency:       10,
					DataSize:        1000,
...
// AnalyzeWasteManagementHierarchies analyzes performance with waste-management-specific hierarchies
func AnalyzeWasteManagementHierarchies(dataset *LifeSnapsDataset) map[string]interface{} {
	...
	analysis["total_bins"] = len(dataset.Bins)
	...
	analysis["performance_predictions"] = map[string]interface{}{
		...
		"estimated_memory_usage_per_bin_kb":   500.0,
...
// GenerateWasteManagementReport creates a comprehensive waste-management dataset report
func GenerateWasteManagementReport(dataset *LifeSnapsDataset, scenarios []TestScenario) string {
	...
	report += fmt.Sprintf("Total Bins: %d\n", len(dataset.Bins))
	...
	for _, bin := range dataset.Bins {
		conditionCount[bin.Condition]++
		genderCount[bin.Gender]++
		riskCount[bin.RiskLevel]++
	}
	
	report += "BIN STATISTICS:\n"
	report += "Conditions:\n"
	for condition, count := range conditionCount {
		pct := float64(count) / float64(len(dataset.Bins)) * 100
	...
	// Hierarchy Analysis
	report += "WASTE_MANAGEMENT HIERARCHY PATTERNS:\n"
...
func (dataset *LifeSnapsDataset) ExportDatasetAsJSON() (string, error) {
	...
	subset := &LifeSnapsDataset{
		Bins:           dataset.Bins[:min(len(dataset.Bins), 100)],
