...
type PerformanceTestSuite struct {
	...
	Parser      *waste-management.WasteManagementParser
...
func NewPerformanceTestSuite() *PerformanceTestSuite {
	...
	params := &hibe.SystemParams{
		MaxDepth: 6, // facility/dept/bin/id/data/access
...
// RunComprehensiveTests executes the full WasteManagement Access Control test scenarios
func (pts *PerformanceTestSuite) RunComprehensiveTests() {
	fmt.Println("=== WasteManagement Access Control Hierarchy Performance Test ===")
...
func (pts *PerformanceTestSuite) runScenario(iterations, concurrent int) {
	...
	// WasteManagement URI test patterns
	testPatterns := []struct {
		uri     string
		pattern string
		isWildcard bool
	}{
		{"/facility/cardiology/bin/12345/vitals/realtime", "/facility/cardiology/bin/12345/vitals/realtime", false},
		{"/facility/neurology/bin/67890/records/historical", "/facility/*/bin/*/records/*", true},
		{"/facility/oncology/bin/11111/imaging/routine", "/facility/*/bin/*/imaging/*", true},
		{"/facility/emergency/bin/22222/vitals/critical", "/facility/emergency/bin/*/vitals/critical", true},
		{"/facility/general/bin/33333/labs/routine", "/facility/*/bin/*/labs/*", true},
	}
...
func (pts *PerformanceTestSuite) testHIBEKeyGeneration(uri string, isWildcard bool) {
	...
	// Parse URI to waste-management pattern
	parsedData, err := pts.Parser.ParseWasteManagementURI(uri)
	if err != nil {
		return
	}
	
	// Create waste-management pattern
	...
	// Generate key
	_, duration, err := pts.HIBEGen.GenerateWasteManagementKey(pattern)
	if err != nil {
		return
	}
...
func (pts *PerformanceTestSuite) testPatternMatching(uri, pattern string) {
	...
	// Perform matching
	isMatch, duration, matches := pts.Matcher.MatchWasteManagementPattern(uri, compiledPattern)
...
func (pts *PerformanceTestSuite) applyWildcardOptimization(pattern *hibe.WasteManagementPattern) {
...
func RunWasteManagementAccessControlTests() {
