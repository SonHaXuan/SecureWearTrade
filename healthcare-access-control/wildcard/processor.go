...
// ComponentType defines the type of healthcare URI component
type ComponentType int

const (
	TypeHospital ComponentType = iota
	TypeDepartment
	TypePatientLabel
	TypePatientID
	TypeDataType
...
func (wp *WildcardProcessor) parseWildcardPattern(patternStr string) (*WildcardPattern, error) {
	...
	if len(parts) != 6 {
		return nil, fmt.Errorf("invalid healthcare pattern: expected 6 components, got %d", len(parts))
	}
...
func (wp *WildcardProcessor) applyAdvancedOptimization(pattern *WildcardPattern) {
	...
	for i := range pattern.Components {
		...
		if !component.IsWildcard {
			...
			case TypeHospital:
				// "hospital" is always the same - use singleton
				component.Value = wp.MemoryOptimizer.GetSingleton("hospital")
			case TypePatientLabel:
				// "patient" is always the same - use singleton
				component.Value = wp.MemoryOptimizer.GetSingleton("patient")
			case TypeDepartment:
...
func (wp *WildcardProcessor) detectComponentType(position int) ComponentType {
	...
	case 2:
		return TypePatientLabel
	case 3:
		return TypePatientID
	case 4:
		return TypeDataType
	case 5:
...
func (wp *WildcardProcessor) GenerateHIBEPattern(wildcardPattern *WildcardPattern) *hibe.HealthcarePattern {
	...
	return &hibe.HealthcarePattern{
		...
		Hospital:     wp.getComponentValue(wildcardPattern, 0),
		Department:   wp.getComponentValue(wildcardPattern, 1),
		PatientID:    wp.getComponentValue(wildcardPattern, 3),
...
func (wp *WildcardProcessor) initializeOptimizationRules() {
	rules := []*OptimizationRule{
		{
			Pattern:          "/hospital/*/patient/*/vitals/*",
			SkipComponents:   []int{1, 3, 5}, // Skip department, patient ID, access level
			MemoryReduction:  0.6,
			SpeedImprovement: 0.75,
			ApplicableTypes:  []ComponentType{TypeDepartment, TypePatientID, TypeAccessLevel},
		},
		{
		...
		{
			Pattern:          "/hospital/cardiology/patient/*/vitals/*",
			SkipComponents:   []int{3, 5}, // Skip patient ID, access level
			MemoryReduction:  0.4,
			SpeedImprovement: 0.5,
			ApplicableTypes:  []ComponentType{TypePatientID, TypeAccessLevel},
		},
	}
...
func NewWildcardMemoryManager() *WildcardMemoryManager {
	...
	for ct := TypeHospital; ct <= TypeAccessLevel; ct++ {
...
func (wmm *WildcardMemoryManager) GetSingleton(value string) []byte {
	// Common healthcare singletons
	switch value {
	case "hospital":
		return []byte("hospital")
	case "patient":
		return []byte("patient")
	default:
