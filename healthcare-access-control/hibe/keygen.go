...
// GenerateHealthcareKey generates a private key for healthcare access patterns
func (kg *HIBEKeyGenerator) GenerateHealthcareKey(pattern *HealthcarePattern) (*PrivateKey, time.Duration, error) {
	start := time.Now()
...
func (kg *HIBEKeyGenerator) generateOptimizedKey(pattern *HealthcarePattern) (*PrivateKey, error) {
	// Get key from pool
	...
	// Algorithm 3: Optimized Key Generation for Healthcare Patterns
...
func (kg *HIBEKeyGenerator) generateComponentKey(component string, depth int) (*big.Int, error) {
	...
	// Fast hash-based key generation optimized for healthcare components
	hash := kg.fastHealthcareHash(component, depth)
...
// fastHealthcareHash provides optimized hashing for healthcare components
func (kg *HIBEKeyGenerator) fastHealthcareHash(component string, depth int) []byte {
	hasher := sha256.New()
	...
	switch {
	case component == "hospital":
		hasher.Write([]byte{0x01}) // Fixed identifier for hospital
	case component == "patient":
		hasher.Write([]byte{0x02}) // Fixed identifier for patient
	case isHealthcareDepartment(component):
		hasher.Write([]byte{0x03}) // Department identifier
		hasher.Write([]byte(component))
	case isPatientID(component):
		hasher.Write([]byte{0x04}) // Patient ID identifier
		hasher.Write([]byte(component))
...
// Helper functions for healthcare component identification
func isHealthcareDepartment(component string) bool {
...
func isPatientID(component string) bool {
	// Simple check for numeric patient IDs
...
func (kg *HIBEKeyGenerator) buildCacheKey(pattern *HealthcarePattern) string {
	var keyParts []string
	for i, component := range pattern.Components {
...
func generatePublicKey(masterKey *MasterKey, params *SystemParams) (*PublicKey, error) {
	// Generate U and H arrays for healthcare hierarchy
